...
function generateTemplateAppealLetter(req: AppealRequest): string {
    ...
    const caseReferences = cases.slice(0, 3).map(c =>
    ...
    const reasonStrategy = reason
        ? reason.appealStrategy.slice(0, 3).map((s, i) => `${i + 1}. ${s}`).join('\n')
...
${req.customInsurerName || insurer?.name || 'Insurance Company'} Appeals Department
...
I formally demand that ${req.customInsurerName || insurer?.name || 'your company'}:
...
Dear ${req.customInsurerName || insurer?.name || 'Insurance Company'} Appeals Department,

I am formally appealing the denial of my claim #${req.claimNumber || '[Claim Number]'} dated ${req.dateOfDenial || '[Date]'} for ${req.procedureDescription || '[medical service]'}, denied as "${req.customReasonName || reason?.name || req.reason || '[reason]'}."
...
Merchant: ${req.customInsurerName || insurer?.name || '[Insurance Company]'}
...
FACTS:
1. I paid $${req.amount?.toLocaleString() || '[Amount]'} for ${req.procedureDescription || 'medical services'} that should have been covered by my insurance policy #${req.policyNumber || '[Number]'} with ${req.customInsurerName || insurer?.name || '[Insurer]'}.
...
export async function generateAppeal(req: AppealRequest, env: { OPENAI_API_KEY?: string }): Promise<AppealResponse> {
    const stateLaw = getStateLaw(req.state) || null;
    const insurer = getInsurer(req.insurer);
    const reason = getDenialReason(req.reason);
    const cases = findRelevantCases(req.insurer, req.reason);
    const deadlineDate = new Date();
    deadlineDate.setDate(deadlineDate.getDate() + (stateLaw?.filingDeadlineDays || 180));
    const responseDate = new Date();
    responseDate.setDate(responseDate.getDate() + 14);

    // Try OpenAI if key available, otherwise use template
    const useAI = env.OPENAI_API_KEY && env.OPENAI_API_KEY !== '';
    ...
    if (useAI) {
        try {
            ...
            appealLetter = data.choices?.[0]?.message?.content || generateTemplateAppealLetter(req);
        } catch {
    ...
    return {
        ...
        insurerName: req.customInsurerName || insurer?.name || '',
        stateLaw,
        relevantCases: cases,
        deadline: deadlineDate.toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' }),
        responseDeadline: responseDate.toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' }),
        confidence: cases.length > 0 && stateLaw ? 85 : cases.length > 0 ? 70 : stateLaw ? 65 : 50,
    };
}
