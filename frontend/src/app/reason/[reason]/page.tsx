...
import {
  denialReasons,
  getDenialReason,
  getAllDenialReasonSlugs,
} from "@/lib/denialReasons";
...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: {
      canonical: `/reason/${params.reason}`,
    },
  };
}
...
export default function ReasonPage({ params }: { params: { reason: string } }) {
  ...
  const relatedCases = caseLaws.filter((c) =>
    reason.relatedCases.includes(c.id),
  );
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-16">
      ...
      <h1 className="text-3xl sm:text-4xl font-bold mb-4">
        Free Appeal for{" "}
        <span className="text-gradient">&ldquo;{reason.name}&rdquo;</span>{" "}
        Denials
      </h1>
      ...
      <div className="glass rounded-2xl p-6 mb-8 flex items-center justify-between flex-wrap gap-4">
        ...
        <Link
          href={`/generate?reason=${reason.slug}`}
          className="px-6 py-3 rounded-xl bg-gradient-to-r from-emerald-500 to-cyan-500 text-white font-bold hover:from-emerald-400 hover:to-cyan-400 transition-all"
        >
      ...
      <div className="prose prose-invert max-w-none">
        <h2 className="text-2xl font-bold text-white mb-4">
          How to Appeal &ldquo;{reason.name}&rdquo; Denials
        </h2>
        <p className="text-slate-300 mb-4">
          When your insurer denies a claim as &ldquo;{reason.name.toLowerCase()}
          ,&rdquo; it means {reason.description.toLowerCase()} Here are proven
          strategies to overturn this type of denial:
        </p>
          ...
          <>
            ...
            <div className="flex flex-wrap gap-2 mb-6">
              {reason.commonCPTCodes.map((code) => (
                <span
                  key={code}
                  className="px-3 py-1 rounded-lg bg-slate-800 text-slate-300 text-sm font-mono"
                >
                  {code}
                </span>
              ))}
        ...
        {relatedCases.length > 0 && (
          <>
            <h2 className="text-2xl font-bold text-white mb-4">
              Winning Cases for &ldquo;{reason.name}&rdquo;
            </h2>
            <div className="space-y-3 mb-6">
              {relatedCases.map((c) => (
                <div key={c.id} className="bg-slate-800/50 rounded-xl p-4">
                  ...
                  <div className="text-emerald-400 text-sm mt-1">
                    Outcome: Won — ${c.amountAwarded.toLocaleString()}
                  </div>
                </div>
              ))}
            </div>
          ...
          <>
            ...
            <div className="flex flex-wrap gap-2 mb-6">
              {reason.relatedStatutes.map((s) => (
                <span
                  key={s}
                  className="px-3 py-1.5 rounded-lg glass text-slate-300 text-sm"
                >
                  {s}
                </span>
              ))}
      ...
      <div className="glass rounded-2xl p-8 text-center">
        <h2 className="text-2xl font-bold mb-3">
          Appeal Your &ldquo;{reason.name}&rdquo; Denial
        </h2>
        ...
        <Link
          href={`/generate?reason=${reason.slug}`}
          className="inline-flex px-8 py-4 rounded-xl bg-gradient-to-r from-emerald-500 to-cyan-500 text-white text-lg font-bold hover:from-emerald-400 hover:to-cyan-400 transition-all pulse-glow"
        >
      ...
      <script
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            "@type": "HowTo",
            name: `How to Appeal "${reason.name}" Insurance Denials`,
            description: `Step-by-step guide to appealing "${reason.name}" insurance denials with ${reason.successRate}% success rate.`,
            step: reason.appealStrategy.map((step, i) => ({
              "@type": "HowToStep",
              position: i + 1,
              text: step,
            })),
