  ...
  const handleSendEmail = async () => {
    ...
    try {
      await fetch(`${API_BASE_URL}/send-email`, {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          to: result.insurerEmail,
          subject: `FORMAL APPEAL - Claim #${form.claimNumber}`,
          body: editedAppeal,
      ...
      <div className="max-w-4xl mx-auto px-4 py-12">
        ...
        <div className="text-center mb-10">
          ...
          <p className="text-slate-400">
            How to appeal denied health insurance claim free — takes less than
            60 seconds
          </p>
        </div>
        ...
        <div className="flex items-center justify-center gap-2 mb-10">
          {[1, 2, 3].map((s) => (
            <div key={s} className="flex items-center gap-2">
              <div
                className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold transition-all ${step >= s ? "bg-gradient-to-r from-sky-500 to-violet-500 text-white glow-blue" : "bg-slate-800 text-slate-500 border border-white/10"}`}
              >
                {s}
              </div>
              {s < 3 && (
                <div
                  className={`w-12 sm:w-20 h-0.5 rounded ${step > s ? "bg-gradient-to-r from-sky-500 to-violet-500" : "bg-slate-700"}`}
                />
              )}
            </div>
          ))}
        ...
        <AnimatePresence mode="wait" custom={direction}>
          ...
          {step === 1 && (
            ...
            >
              <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                  <h2 className="text-xl font-bold text-white">
                    Enter Denial Details
                  </h2>
                  <p className="text-slate-400 text-sm">
                    Fill in the information from your denial letter
                  </p>
                </div>
              </div>
              ...
              <div className="grid sm:grid-cols-2 gap-4 mb-6">
                <div>
                  ...
                  <select
                    value={form.insurer}
                    onChange={(e) =>
                      setForm((p) => ({ ...p, insurer: e.target.value }))
                    }
              ...
              <div className="mb-6">
                <label className="block text-sm text-slate-300 mb-1.5">
                  Denial Reason<span className="text-red-400">*</span>
                </label>
                <select
                  value={form.reason}
                  onChange={(e) =>
                ...
                <div className="grid sm:grid-cols-1 gap-4 mb-6 relative pl-4 border-l-2 border-slate-700/50">
                  {form.insurer === "other" && (
                    <InputField
                      ...
                      required
                      value={form.customInsurerName || ""}
                      onChange={(v) => updateForm("customInsurerName", v)}
                    />
                  )}
                  {form.state === "other" && (
                    <InputField
                      label="State or Country Name"
                      name="customStateName"
                      placeholder="e.g. New York, India"
                      required
                      value={form.customStateName || ""}
                      onChange={(v) => updateForm("customStateName", v)}
                    />
                  )}
                  {form.reason === "other" && (
                    <InputField
                      label="Specific Denial Reason"
                      name="customReasonName"
                      placeholder="e.g. Missing Records, Out of Network"
                      required
                      value={form.customReasonName || ""}
                      onChange={(v) => updateForm("customReasonName", v)}
                    />
                  )}
                </div>
              )}
              ...
              <div className="grid sm:grid-cols-2 gap-4 mb-6">
                ...
                <InputField
                  ...
                  onChange={(v) => updateForm("claimNumber", v)}
                />
                <InputField
                  label="Patient Name"
                  name="patientName"
                  ...
                  onChange={(v) => updateForm("patientName", v)}
                />
                <InputField
                  ...
                  onChange={(v) => updateForm("dateOfService", v)}
                />
                <InputField
                  ...
                  type="date"
                  placeholder=""
                  value={form.dateOfDenial || ""}
                  onChange={(v) => updateForm("dateOfDenial", v)}
                />
              </div>
              ...
              <InputField
                ...
                placeholder="e.g., MRI of left knee, physical therapy sessions"
                value={form.procedureDescription || ""}
                onChange={(v) => updateForm("procedureDescription", v)}
              />
              ...
              <button
                ...
                disabled={
                  !form.insurer ||
                  (form.insurer === "other" && !form.customInsurerName) ||
                  !form.state ||
                  (form.state === "other" && !form.customStateName) ||
                  !form.reason ||
                  (form.reason === "other" && !form.customReasonName)
                }
            ...
            >
                ...
                {[
                  {
                    ...
                    value:
                      form.insurer === "other"
                        ? form.customInsurerName
                        : insurers.find((i) => i.slug === form.insurer)?.name ||
                          form.insurer,
                    icon: Building2,
                  },
                  {
                    ...
                    value:
                      form.state === "other"
                        ? form.customStateName
                        : stateLaws.find((s) => s.code === form.state)?.name ||
                          form.state,
                    icon: Shield,
                  },
                  {
                    ...
                    value:
                      form.reason === "other"
                        ? form.customReasonName
                        : denialReasons.find((r) => r.slug === form.reason)
                            ?.name || form.reason,
                    icon: AlertCircle,
                  },
                  {
                    label: "Amount",
                    value: `$${form.amount?.toLocaleString() || "0"}`,
                    icon: FileText,
                  },
                ].map((item) => (
                  <div
              ...
              <div className="bg-slate-800/30 rounded-xl p-4 mb-6 border border-sky-500/20">
                ...
                <p className="text-slate-300 text-sm">
                  Your appeal will include:{" "}
                  <strong className="text-white">
                    {stateLaws.find((s) => s.code === form.state)
                      ?.statuteCitation || "applicable state law"}
                  </strong>{" "}
                  citation, relevant case precedents, and a 14-day demand
                  deadline with regulatory action warning.
                </p>
              </div>
              ...
              <div className="flex gap-3">
                <button
                  onClick={() => goTo(1)}
                  className="px-6 py-4 rounded-xl glass text-slate-300 font-semibold hover:bg-white/10 transition-all flex items-center gap-2"
                >
                ...
                >
                  {loading ? (
                    ...
                    <>
                      <Zap className="w-5 h-5" /> Generate Appeal Letter
                    </>
                  )}
                </button>
              </div>
            ...
            >
              ...
              <div className="glass rounded-2xl p-4 mb-4 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div
                    className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm ${result.confidence >= 70 ? "bg-emerald-500/20 text-emerald-400" : "bg-amber-500/20 text-amber-400"}`}
                  >
                    {result.confidence}%
                  </div>
                  <div>
                    <div className="text-white font-semibold">
                      Appeal Confidence Score
                    </div>
                    <div className="text-slate-400 text-xs">
                      Based on state law match and case precedents
                    </div>
                  </div>
                </div>
                <div className="text-right">
                  <div className="flex flex-col items-end gap-2">
                    <div className="flex items-center gap-2 bg-slate-800/50 px-3 py-1.5 rounded-lg border border-white/5 shadow-inner transition-all hover:bg-slate-800">
                      <div className="text-right">
                        <span className="text-slate-500 uppercase font-bold text-[9px] block leading-none mb-0.5">
                          Filing Deadline
                        </span>
                        <span className="text-sky-400 font-bold text-xs">
                          {result.deadline}{" "}
                          <span className="text-[10px] font-medium text-slate-500">
                            ({result.stateLaw?.filingDeadlineDays || 180} days)
                          </span>
                        </span>
                      </div>
              ...
              <div className="glass rounded-2xl p-6 sm:p-8 mb-4">
                <div className="flex items-center justify-between mb-4">
                  <h2 className="text-xl font-bold text-white flex items-center gap-2">
                    <FileText className="w-5 h-5 text-sky-400" /> Appeal Letter
                  </h2>
                  <div className="flex gap-2">
                    <button
                      onClick={() => setIsEditingAppeal(!isEditingAppeal)}
                      className="px-3 py-1.5 rounded-lg bg-slate-700 text-sm text-slate-300 hover:bg-slate-600 flex items-center gap-1"
                    >
                      {isEditingAppeal ? (
                      ...
                      {isEditingAppeal ? "Done" : "Edit"}
                    </button>
                    <button
                      onClick={() => copyText(editedAppeal, "letter")}
                      className="px-3 py-1.5 rounded-lg bg-slate-700 text-sm text-slate-300 hover:bg-slate-600 flex items-center gap-1"
                    >
                      {copied === "letter" ? (
                        <CheckCircle className="w-4 h-4 text-emerald-400" />
                    ...
                    <button
                      onClick={() =>
                        downloadPDF(
                          editedAppeal,
                          `appeal-letter-${form.claimNumber || "draft"}.pdf`,
                        )
                      }
                      className="px-3 py-1.5 rounded-lg bg-slate-700 text-sm text-slate-300 hover:bg-slate-600 flex items-center gap-1"
                    >
                ...
                {isEditingAppeal ? (
                  ...
                  <pre className="whitespace-pre-wrap text-sm text-slate-300 bg-slate-800/50 rounded-xl p-4 max-h-96 overflow-y-auto font-mono leading-relaxed">
                    {editedAppeal}
                  </pre>
                )}
              </div>
              ...
              <div className="glass rounded-2xl p-6 mb-4">
                <h3 className="text-lg font-bold text-white mb-3 flex items-center gap-2">
                  <Mail className="w-5 h-5 text-emerald-400" /> One-Click Send
                  to {result.insurerName}
                </h3>
                <p className="text-slate-400 text-sm mb-4">
                  Send your appeal directly to{" "}
                  <strong className="text-white">
                    {result.insurerEmail || "the insurer"}
                  </strong>{" "}
                  via email.
                </p>
                <button
                  ...
                  disabled={
                    sending || sent || !result.insurerEmail || isHovered
                  }
                ...
                >
                  {sent ? (
                    ...
                    <>
                      <Mail className="w-5 h-5" /> Email Not Available for
                      Custom Insurer
                    </>
                  ) : sending ? (
                    <>
                      <Loader2 className="w-5 h-5 animate-spin" /> Sending to{" "}
                      {result.insurerName}...
                    </>
                  ) : (
                    <>
              ...
              <div className="glass rounded-2xl p-6 mb-4">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="text-lg font-bold text-white flex items-center gap-2">
                    <Shield className="w-5 h-5 text-amber-400" /> Bank
                    Chargeback Letter
                  </h3>
                  <div className="flex gap-2">
                    <button
                    ...
                    <button
                      onClick={() => copyText(editedBank, "bank")}
                      className="px-3 py-1.5 rounded-lg bg-slate-700 text-sm text-slate-300 hover:bg-slate-600 flex items-center gap-1"
                    >
                      {copied === "bank" ? (
                        <CheckCircle className="w-4 h-4 text-emerald-400" />
                    ...
                    <button
                      onClick={() =>
                        downloadPDF(
                          editedBank,
                          `chargeback-letter-${form.claimNumber || "draft"}.pdf`,
                        )
                      }
                      className="px-3 py-1.5 rounded-lg bg-slate-700 text-sm text-slate-300 hover:bg-slate-600 flex items-center gap-1"
                    >
                ...
                {isEditingBank ? (
                  ...
                  <pre className="whitespace-pre-wrap text-sm text-slate-300 bg-slate-800/50 rounded-xl p-4 max-h-48 overflow-y-auto font-mono">
                    {editedBank}
                  </pre>
                )}
              </div>
              ...
              <div className="text-center">
                <button
                  ...
                  className="px-6 py-3 rounded-xl glass text-slate-300 font-semibold hover:bg-white/10 transition-all"
                >
      ...
      <script
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            "@type": "WebApplication",
            name: "Free AI Appeal Letter Generator",
            description:
              "Free AI-powered insurance denial appeal letter generator with state law citations and case precedents.",
            url: "https://appeallettergenerator.com/generate",
